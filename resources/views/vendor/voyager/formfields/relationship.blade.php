@if(isset($options->model) && isset($options->type))
    ...
    @if(class_exists($options->model))
        ...
        @if($options->type == 'belongsTo')
            ...
            @else
            @if($options->column == "main_cat") 
            @php
            $query = \App\MainMediaCategory::get();
            @endphp
            <select
                ...
                @elseif($options->column == "sub_cat")
                @php
                $query2 = \App\MediaSubcategory::where('main_cat_name',$dataTypeContent->main_cat)->get();
                @endphp
                <select class="form-control" name="{{ $options->column }}"
                        @if(!is_null($dataTypeContent->getKey())) data-id="{{$dataTypeContent->getKey()}}" @endif
                        data-method="{{ !is_null($dataTypeContent->getKey()) ? 'edit' : 'add' }}"
                        @if($row->required == 1) required @endif
                    >
                    @if(!is_null($dataTypeContent->getKey()))
                        @foreach($query2 as $relationshipData)
                            <option value="{{ $relationshipData->{$options->key} }}" @if(old($options->column, $dataTypeContent->{$options->column}) == $relationshipData->{$options->key}) selected="selected" @endif>{{ $relationshipData->{$options->label} }}</option>
                        @endforeach
                    @endif
                </select>
