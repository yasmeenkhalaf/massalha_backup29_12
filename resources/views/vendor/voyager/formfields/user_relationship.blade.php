        ...
        @if($options->type == 'belongsTo')
            ...
            @else
                ...
                >
                    ...
                    @if(Auth::user()->role_id!="1" && $row->field == "user_belongsto_role_relationship" && $relationshipData->{$options->key} != "1")
                        <option value="{{ $relationshipData->{$options->key} }}" @if(old($options->column, $dataTypeContent->{$options->column}) == $relationshipData->{$options->key}) @endif>{{ $relationshipData->{$options->label} }}</option>
                    @endif
                    @endforeach
                </select>
